...
document.getElementById('pensionForm').addEventListener('submit', function(e) {
  ...
  const prevWorkYrs = hasPrevWork ? Math.max(1, parseInt(document.getElementById('prevWorkYears').value) || 1) : 0;
  ...
  const startAgeYears = join.years;
...
<div class="info-card">
  ...
  <div class="info-grid">
    ...
    <div class="info-row"><span class="info-label">عمر الانتساب:</span><span class="info-value">${join.years} سنة و ${join.months} شهر و ${join.days} يوم</span></div>
    ${hasPrev ? `<div class="info-row"><span class="info-label">فئة قديمة:</span><span class="info-value">فئة ${prevCat}</span></div>` : ''}
    ${hasPrevWork ? `<div class="info-row"><span class="info-label">خدمة سابقة:</span><span class="info-value">${prevWorkYrs} سنة</span></div>` : ''}
  </div>
</div>`;
  ...
  plan.forEach((p, idx) => {
    ...
    const rows            = formatTableRows(table, joinDate, by, bm, bd);
...
<div class="scenario-wrap">
  ...
  <div class="scenario-meta" style="border-color:${sc.border};background:#fafcff;">
    ...
    <div class="meta-item"><span class="lbl">عمر بداية الجدول: </span><span class="val">${join.years} سنة و ${join.months} شهر و ${join.days} يوم</span></div>
...
<div class="export-buttons">
  ...
  window.lastResults = { name, gender, by, bm, bd, jy, jm, jd, plan, html, hasPrev, prevCat, hasPrevWork, prevWorkYrs };
});
    ...
    loadScript('https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.2.7/vfs_fonts.min.js', function() {
      loadFont('amiri-regular', 'Amiri-Regular.ttf', function(reg) {
        loadFont('amiri-bold', 'Amiri-Bold.ttf', function(bold) {
          ...
          const { by, bm, bd, jy, jm, jd, hasPrev, prevCat, hasPrevWork, prevWorkYrs } = d;
          const joinDate   = new Date(jy, jm - 1, jd);
          const today      = new Date();
          const now        = calculateExactAge(by, bm, bd, today.getFullYear(), today.getMonth()+1, today.getDate());
          const join       = calculateExactAge(by, bm, bd, jy, jm, jd);
